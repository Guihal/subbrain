...
// Layer 2: Context
const ctx = db.query("SELECT * FROM layer2_context ORDER BY updated_at DESC").all() as any[];
console.log(`\n[Layer 2: Context] ${ctx.length} entries`);
for (const c of ctx)
  console.log(`  - [${c.agent_id || "none"}] ${c.title?.slice(0, 80)} (tags: ${c.tags})`);

// Layer 3: Archive
const arch = db.query("SELECT * FROM layer3_archive ORDER BY updated_at DESC").all() as any[];
...
// Embeddings
try {
  const embeds = db.query("SELECT count(*) as cnt FROM embeddings").get() as any;
...
// FTS indices check
try {
  const ftsCtx = db.query("SELECT count(*) as cnt FROM fts_context").get() as any;
  const ftsArch = db.query("SELECT count(*) as cnt FROM fts_archive").get() as any;
  const ftsShared = db.query("SELECT count(*) as cnt FROM fts_shared").get() as any;
