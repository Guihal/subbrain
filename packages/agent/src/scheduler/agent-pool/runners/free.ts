...
export async function runFreeTask(
  agentLoop: AgentLoop,
  task: AgentTaskRecord,
): Promise<RunnerResult> {
  const log = logger.child("pool.runner");
  const maxTokens = Number(
    process.env.AGENT_POOL_MAX_TOKENS_FREE ?? process.env.AGENT_POOL_MAX_TOKENS_PER_TASK ?? 60_000,
  );
  let consumed = 0;
  ...
  try {
    ...
    if (
      ...
      reason.includes("token_budget_exceeded") ||
      reason.includes("AbortError")
    ) {
      log.warn("token_budget_exceeded", {
        meta: { task_id: task.id, type: task.type, consumed, cap: maxTokens },
      });
