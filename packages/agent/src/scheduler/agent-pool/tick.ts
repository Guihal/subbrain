...
export async function runTick(deps: PoolDeps): Promise<void> {
  ...
  if (deps.router.isOverloaded) {
    deps.log.info("router overloaded, skip");
    return;
  }
  ...
  const results = await Promise.allSettled(tasks.map(({ task }) => runSingle(deps, task)));

  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    if (r.status === "rejected") {
      deps.log.error("runFn rejected", {
        meta: { id: tasks[i].task.id, reason: String(r.reason) },
      });
