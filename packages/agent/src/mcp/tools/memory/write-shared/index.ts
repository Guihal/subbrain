...
export function writeShared(
  ...
  const expiresAt =
    typeof params.expires_at === "number" ? params.expires_at : defaultExpiresAt("shared", params.category);
  ...
  if (rag) {
    try {
      ...
      if (dd.action === "reject") {
        ...
        const newId = await insertAndSupersede(deps, svc, rag, params, kind, expiresAt, dd.supersedesId, signal);
