...
export function doneWithArtifact(args: DoneWithArtifactArgs): ToolResult {
  if (terminated) {
    return {
      success: false,
      error: { code: "already_terminated", message: "Session already terminated" },
    };
  }
  ...
  if (args.status === "complete" && (!args.artifact || args.artifact.trim().length === 0)) {
    return {
      success: false,
      error: { code: "missing_artifact", message: "status=complete requires artifact" },
    };
  }
  ...
  if (args.status === "failed" && (!args.reason || args.reason.trim().length === 0)) {
    return {
      success: false,
      error: { code: "missing_reason", message: "status=failed requires reason" },
    };
  }
