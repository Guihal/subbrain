...
export function getBlockByLabel(db: Database, ownerRole: string, label: string): BlockRow | null {
  return (
    db
...
export function listBlocks(db: Database, limit?: number, offset?: number): BlockRow[] {
  const sql =
    `SELECT id, owner_role, label, body, created_at, updated_at, version
     FROM memory_blocks
