...
export class AgentTasksTable {
  ...
  insertPending(input: EnqueueInput): number {
    const result = this.db
      ...
      .run(input.type, input.prompt, input.priority ?? 0, input.scheduledAt ?? null, input.createdBy);
  ...
  markRunning(id: number, now: number): void {
    this.db.query(`UPDATE agent_tasks SET status = 'running', started_at = ? WHERE id = ?`).run(now, id);
  }
