...
export class ArbitrationTranscriptRepository {
  ...
  insert = (row: Omit<ArbitrationTranscriptRow, "id">): string => this.table.insert(row);

  getById = (id: string): ArbitrationTranscriptRow | null => this.table.getById(id);

  listByRoom = (
    roomId: string,
    opts?: { limit?: number; offset?: number },
  ): { items: ArbitrationTranscriptRow[]; total: number } => this.table.listByRoom(roomId, opts);
}
