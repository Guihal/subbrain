...
function readEnv(): Cfg {
  ...
  const a = Number.parseInt(process.env.ARCHIVE_PROMOTE_MIN_ACCESS ?? "5", 10);
  const c = Number.parseFloat(process.env.ARCHIVE_PROMOTE_MIN_CONFIDENCE ?? "0.7");
  const lim = Number.parseInt(process.env.CROSS_LAYER_DEDUP_LIMIT ?? "200", 10);
  return {
    enabled,
...
async function promoteArchiveToShared(
  deps: CrossLayerDeps,
  cfg: Cfg,
  ...
  for (const arc of candidates) {
    ...
    try {
      ...
      log.warn("promote failed", {
        meta: { archive_id: arc.id.slice(0, 8), msg: (err as Error).message },
      });
    }
  }
...
export async function runCrossLayerDedup(deps: CrossLayerDeps): Promise<CrossLayerResult> {
  ...
  for (const s of settled) {
    if (s.status === "fulfilled") {
      ...
      r.errors++;
      log.warn("pair failed", { meta: { msg: String(s.reason) } });
    }
  }
  ...
  r.errors += p.errors;
  log.info(
    `done: pairs=${r.pairs_examined} supersedes=${r.supersedes_added} promoted=${r.promoted_to_shared} errors=${r.errors}`,
  );
  return r;
}
