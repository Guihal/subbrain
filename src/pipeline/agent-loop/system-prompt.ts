...
export async function buildAgentSystemPrompt(
  ...
  const schedulerLine = schedule
    ...
    : `Тебя запустил пользователь разово через /v1/autonomous (interactive, следующего авто-запуска не будет). Доведи задачу до конца в этом вызове.`;
...
Ты **НЕ** в чате с пользователем. Пользователь сейчас **не за компьютером** (спит, занят, не в сети). Имя — из shared_memory.
${schedulerLine}
...
### ⚠️ Антипаттерны \`done\` (все режимы)

- **0 вызовов memory_write AND 0 вызовов tg_send_message** за сессию → состояние мира не изменено, \`done\` запрещён.
