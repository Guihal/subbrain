/**
 * Code-tool input validators applied at create/edit time. Single combined
 * entry point: `applyCodeToolGuards(code, name, log)` returns null on pass
 * or `{success:false, error}` on hard reject.
 *
 * Two layers:
 *  - SANDBOX_FORBIDDEN — sandbox compat (Worker has no require/import). Hard
 *    reject. Same patterns as the original `checkSandboxCompat` previously
 *    inlined в code-mgmt.tools.ts.
 *  - HARDCODED_FACT_PATTERNS (F-2) — blocks frozen client snapshots that
 *    cause poisoning of TG-digest output. ≥2 distinct patterns matched →
 *    hard reject; 1 match → warn-and-accept; 0 → ok.
 *
 * Background: F-2 in `docs/tasks/code-tools-poisoning-fix.md`. Diagnosis:
 * `~/vault/RLM/Daily/2026-04-28.md`.
 */
...
/**
 * F-2 patterns. Each tuned to require BOTH a key word AND the offending
 * literal — bare numbers / generic dates / single emojis don't match.
 */
