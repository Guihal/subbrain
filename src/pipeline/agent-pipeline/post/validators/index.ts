...
import {
  WHITELIST_SHARED, WHITELIST_CONTEXT, BLACKLIST_CATEGORY_PREFIXES,
  BLACKLIST_CONTENT_REGEXES, MAX_SHARED_CONTENT, MAX_CONTEXT_CONTENT,
...
export function validateCategoryAndContent(
  ...
  if (!allowed.has(cat))
    return { ok: false, reason: `category '${category}' not in ${layer} whitelist (${[...allowed].join(",")})` };
  for (const bad of BLACKLIST_CATEGORY_PREFIXES)
    if (cat.startsWith(bad)) return { ok: false, reason: `category '${category}' starts with blacklisted prefix '${bad}'` };
  ...
  if (content.length > cap)
    return { ok: false, reason: `content too long (${content.length} > ${cap}); summarise or use layer3_archive` };
  for (const re of BLACKLIST_CONTENT_REGEXES)
    if (re.test(content)) return { ok: false, reason: `content matches blacklisted pattern (${re.source})` };
  return { ok: true };
}
...
export function validateExpiresAt(
  ...
  if (TIME_BOUND_CATEGORIES.has(cat) && !hasValue)
    return { ok: false, reason: `expires_at required for category '${category}' (unix seconds; e.g. Math.floor(Date.now()/1000) + 30*86400 for +30d)` };
  if (!hasValue) return { ok: true };
  const v = expiresAt as number;
  if (!Number.isInteger(v)) return { ok: false, reason: `expires_at must be integer unix seconds, got ${v}` };
  if (v >= 1e12) return { ok: false, reason: `expires_at must be unix seconds (Math.floor(Date.now()/1000)+...), not ms; got ${v}` };
  if (v <= nowSec + 60) return { ok: false, reason: `expires_at must be > now+60s (now=${nowSec}, got ${v})` };
  return { ok: true };
}
