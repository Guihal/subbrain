...
export class MemoryTable {
  ...
  // Mirror of layer1_focus written by the sleep-time rewriter night-cycle
  // step. Real layer1_focus remains source-of-truth for system-prompt.ts;
  // shadow exists so a human can diff proposed rewrites for weeks before any
  // flip. Same KV shape, no FTS, no triggers. Methods mirror getFocus /
  // setFocus / getAllFocus / deleteFocus 1:1.

  getShadowFocus(key: string): string | null {
    const row = this.db
  ...
  setShadowFocus(key: string, value: string): void {
    this.db
      .query(
  ...
  getAllShadowFocus(): Record<string, string> {
    const rows = this.db
  ...
  clearShadowFocus(): void {
    this.db.query("DELETE FROM layer1_focus_shadow").run();
  }

  /**
   * M-11: top-K shared_memory rows ranked for focus-block rewrite.
   * persona kind weighted ×1.5; salience baseline 0.5; access_count
   * folded log-style so very-popular rows pull but don't dominate. Filters
   * to status='active' (pending / rejected / superseded skipped). Used
   * only by `night-cycle/steps/focus-rewrite.ts`.
   */
  selectTopSharedForFocusRewrite(
    limit: number,
  ): { id: string; category: string; kind: string; content: string; salience: number | null; last_accessed_at: number | null; access_count: number }[] {
    return this.db
      .query<
        {
          id: string;
