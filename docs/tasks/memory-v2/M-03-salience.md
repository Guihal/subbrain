...
- `src/db/schema.ts` — Migration **13** (assigned). `ALTER TABLE … ADD COLUMN salience REAL NOT NULL DEFAULT 0.5` на 3 таблицы. Idempotent (дублирующая колонка → catch). `db.transaction()` + per-statement `.run()`.
- `src/db/types.ts` — `salience?: number` на `SharedRow` / `ContextRow` / `ArchiveRow`. Optional т.к. legacy SELECTs могут не запросить.
- `src/repositories/memory.repo.ts` — расширить `bumpAccess(layer, ids)` чтобы UPDATE дополнительно обновлял `salience`. Использовать SQL CASE для bonus формулы:
  ```sql
  UPDATE <table>
...
- `src/pipeline/night-cycle/steps/decay-salience.ts` — **NEW** файл (≤100 LOC). Step класса `NightCycleStep` (если есть базовый интерфейс — match'ить). Выполняет на 3 таблицах:
  ```sql
  UPDATE <table>
...
- `src/pipeline/night-cycle/steps/index.ts` — добавить `decaySalience` в order, после `runMemoryDedup` (соседство со step'ами которые трогают memory rows).
- `src/rag/pipeline.ts` — в rerank-score функции (после persona boost от M-07): `score *= (1 + 0.1 * (r.salience ?? 0.5))`. Optional поле — context/archive/shared всегда имеют salience после mig 13; default 0.5 если undefined.
- `src/db/tables/{shared,memory}.ts` (если SELECT-list явный) — добавить `salience` колонку. Если `SELECT *` — пропустить.
...
5. `grep -n "salience" src/repositories/memory.repo.ts src/rag/pipeline.ts src/pipeline/night-cycle/steps/decay-salience.ts` → ≥3 hits.
