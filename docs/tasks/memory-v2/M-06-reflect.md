...
- `packages/agent/src/pipeline/night-cycle/steps/reflect.ts` — **NEW** ≤200 LOC. Step функция `runReflect(deps): Promise<ReflectResult>` где `ReflectResult = { groups_examined, facts_promoted, edges_created, llm_failures }`. Использует:
...
- `packages/agent/src/pipeline/night-cycle/steps/index.ts` — добавить `runReflect` в order, **после** `runMemoryDedup` и `decaySalience` (чтобы reflect видел уже задедуплицированные + decayed rows).
- `packages/agent/src/pipeline/night-cycle/post-steps.ts` (если используется для wiring) — wire.
- `packages/agent/src/pipeline/night-cycle/types.ts` — `ReflectResult` если нужен для общего типа.
...
4. `grep -n "runReflect\|night.reflect" packages/agent/src/pipeline/night-cycle/steps/reflect.ts packages/agent/src/pipeline/night-cycle/steps/index.ts packages/agent/src/pipeline/night-cycle/post-steps.ts` → ≥3 hits.
5. `grep -n "REFLECT_ENABLED\|REFLECT_MIN_ACCESS\|REFLECT_MIN_GROUP" packages/agent/src/pipeline/night-cycle/steps/reflect.ts` → ≥3 hits.
