...
- `packages/server/src/routes/memory.ts` — 0 endpoints для edges.
...
- `packages/server/src/routes/memory.ts` — добавить 2 endpoint'а (≤30 LOC). Use existing `paginate()` envelope helper. Под `authMiddleware`.
- `packages/core/src/repositories/edges.repo.ts` — если `getRelated` не подходит (returns `{id,layer}[]` без kind/weight), добавить `getRelatedDetailed(id, layer, kinds?)` returning `EdgeRow[]`. ≤20 LOC delta. **Если уже подходит** — skip (use existing).
...
**НЕ трогать:**
- `packages/core/src/db/tables/edges.ts` — schema layer, M-05 territory.
- `packages/agent/src/mcp/tools/memory-curation-tools.ts` (write surface через MCP — already exists).
...
`packages/server/src/routes/memory.ts` — два GET:
...
4. `grep -n "/edges" packages/server/src/routes/memory.ts` → ≥2 hits (2 routes).
...
- `grep -n "/edges" packages/server/src/routes/memory.ts` → 2 hits ✅
