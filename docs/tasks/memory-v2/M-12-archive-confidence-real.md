...
- `packages/core/src/db/schema.ts` — Migration **15** (assigned). SQLite не позволяет ALTER COLUMN type → нужен rebuild через temp table + INSERT SELECT + DROP + RENAME. Pattern matches Migration 3/7 (layer4_log_new). Под `db.transaction()` + per-statement `.run()`. Idempotent guard `if (version < 15)`.
- `packages/core/src/db/types.ts` — `ArchiveRow.confidence: number | null` (заменить TEXT-union).
...
- `packages/agent/src/pipeline/night-cycle/steps/anti-patterns-step.ts` (или эквивалент) — это писатель в archive с HIGH/LOW. Mapping 'HIGH' → 0.9, 'LOW' → 0.4.
- `packages/agent/src/pipeline/night-cycle/process-session.ts` (если пишет в archive) — same.
- `packages/server/src/routes/memory.ts` — admin endpoint `/v1/memory/archive` PATCH/POST validation: TypeBox `t.Optional(t.Number({minimum:0, maximum:1}))` вместо string-enum.
