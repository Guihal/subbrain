...
```json
{
  ...
  "escalation_triggers": [
    "AuthService imports anything from src/lib/* or packages/core/src/db/* — currently it only imports node:crypto, but verify before move; if true, that import target must already live in core or this packet aborts → FAIL: auth-service-non-leaf.",
    ...
    "tests/auth-service.test.ts or tests/auth.test.ts fails after move (they import from packages/agent/src/services/auth.service — must rewrite to @subbrain/core) → FAIL: test-import-not-rewritten."
  ],
  "glossary": {
    "shim file": "the original-path file (e.g. packages/providers/src/types.ts) replaced with `export * from \"@subbrain/core/types/providers\";` — keeps unrewritten importers compiling until A1-3..A1-7 finish rewriting them.",
...
{
  "task_id": "A1-3",
  "goal": "Move the rest of @subbrain/core (packages/core/src/db/, packages/core/src/repositories/, remaining src/lib/*) and retire A1-2 shims where importers have been rewritten.",
  "non_goals": [
    "Do not move src/lib/model-router*, packages/providers/src/rate-limiter.ts, src/lib/personas* (those go to providers/agent in later packets).",
  ...
  "read_context": [
    ...
    "packages/core/src/lib/model-map/openai-compat-overrides.ts",
    "packages/core/src/lib/clock.ts",
    "packages/core/src/lib/errors.ts",
    "packages/core/src/lib/memory-decay.ts",
    "packages/core/src/lib/metrics.ts",
    "src/lib/metrics/",
    "packages/core/src/lib/redact.ts",
    "packages/core/src/lib/sse.ts",
  ...
  "acceptance": [
    ...
    "test ! -e packages/core/src/lib/http-client.ts",
    "bun install",
    "bunx tsc --noEmit",
    "bunx tsc -p packages/core/tsconfig.json --noEmit",
    "bun test",
...
{
  "task_id": "A1-4",
  "goal": "Move provider router and rate limiter into packages/providers; rewrite imports to @subbrain/providers; retire the packages/providers/src/types.ts shim left by A1-2 (since packages/providers/src/ is now empty).",
  "non_goals": [
    "Do not move anything outside packages/providers/src/, packages/providers/src/rate-limiter.ts, packages/core/src/lib/model-router.ts, packages/providers/src/model-router/.",
    "Do not edit any provider streaming logic, SSE parser, or fallback policy code beyond import strings.",
    "Do not adjust the model fallback chain in packages/core/src/lib/model-map.ts (already in core).",
  ...
  "acceptance": [
    ...
    "test ! -e packages/providers/src/rate-limiter.ts",
    "test ! -e packages/core/src/lib/model-router.ts",
...
```json
{
  ...
  "non_goals": [
    "Do not move packages/agent/src/mcp/, packages/agent/src/scheduler/, packages/agent/src/rag/, packages/agent/src/telegram/, src/lib/personas* in this packet — those land in A1-6b/c/d.",
  ...
  "rollback": "git reset --hard HEAD~1 and bun install.",
  "escalation_triggers": [
    "Any pipeline or service file imports from packages/server/src/routes/ or packages/server/src/app/ — would create cycle once server is built → FAIL: package-cycle.",
...
{
  "task_id": "A1-6c",
  "goal": "Move packages/agent/src/scheduler/ and packages/agent/src/telegram/ into packages/agent, retaining free-agent.ts AppDeps import for A1-7a cycle break.",
  "non_goals": [
...
```json
{
  ...
  "acceptance": [
    ...
    "test ! -e src/rag",
    "test ! -e packages/agent/src/lib/personas.ts",
  ...
  "glossary": {
    "personas coexistence": "packages/agent/src/lib/personas.ts and src/lib/personas/ already coexist on disk pre-A1. Linux ext4 supports this. After move, packages/agent/src/personas.ts and packages/agent/src/personas/ also coexist.",
...
```json
{
  ...
  "risk_tier": "public-api",
  "acceptance": [
    "! grep -rn 'packages/agent/src/services/' CLAUDE.md AGENTS.md README.md",
    "! grep -rn 'packages/agent/src/pipeline/' CLAUDE.md AGENTS.md README.md docs/tasks/runtime-arch/A2-plugin-runtime.md",
    "! grep -rn 'packages/server/src/routes/' CLAUDE.md AGENTS.md README.md",
    "! grep -rn 'packages/core/src/db/' CLAUDE.md AGENTS.md README.md",
