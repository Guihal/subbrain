...
- Грепать сборку messages в `packages/server/src/routes/chat.ts`, `AgentPipeline.execute`, `AgentLoop.run`.
...
- Место: `packages/agent/src/pipeline/agent-loop/tool-runner.ts` (scope-map), `packages/core/src/lib/http-client.ts` (если оттуда дёргается).
...
- Поднять:
  - `packages/core/src/lib/http-client.ts` default 60s → 180s.
  - Copilot streams 180s → 300s.
  - NVIDIA non-stream — отдельный scope 240s.
- Tool scopes (`packages/agent/src/pipeline/agent-loop/tool-runner.ts`):
  - `consult_*` 20s → 60s.
