...
### Тулза
- Регистрация: `src/mcp/registry/telegram.tools.ts`, `scope: "agent-only"`.
- Вход: `{ query: string, chat_id?: string, from?: string, to?: string, limit?: number }` (`from`/`to` — ISO даты, `limit` default 20).
- Логика: `src/mcp/telegram-tools.ts` — FTS5 по отдельному индексу TG-сообщений (или переиспользовать `raw_log` FTS если TG туда пишется).
...
- Sanitize запросов через `src/lib/fts-utils.ts:sanitizeFtsQuery`.
...
- `src/mcp/telegram-tools.ts` (helper для чтения inbox + `searchMessages` для FTS)
- `src/mcp/registry/telegram.tools.ts` (регистрация `telegram_search`)
- `src/db/schema.ts` (таблица `tg_messages` + FTS, если ещё нет)
