...
- `packages/agent/src/mcp/telegram-tools.ts` уже умеет отправлять и (через MTProto userbot) читать сообщения.
...
Файл: `packages/agent/src/scheduler/telegram-poller.ts` (новый).

- Экспортирует `startPoller()`, `stopPoller()`, `pollerRunning` guard (аналогично `nightCycleRunning`).
- В `packages/server/src/index.ts` при старте (рядом с night-cycle-scheduler): если `process.env.TG_POLLER === "true"` → `startPoller()`. В shutdown-блоке → `stopPoller()`.
...
### Тулза
- Регистрация: `packages/agent/src/mcp/registry/telegram.tools.ts`, `scope: "agent-only"`.
- Вход: `{ query: string, chat_id?: string, from?: string, to?: string, limit?: number }` (`from`/`to` — ISO даты, `limit` default 20).
- Логика: `packages/agent/src/mcp/telegram-tools.ts` — FTS5 по отдельному индексу TG-сообщений (или переиспользовать `raw_log` FTS если TG туда пишется).
...
- `packages/agent/src/scheduler/telegram-poller.ts` (новый)
- `packages/server/src/index.ts` (старт/стоп, cleanup на shutdown)
- `packages/agent/src/mcp/telegram-tools.ts` (helper для чтения inbox + `searchMessages` для FTS)
- `packages/agent/src/mcp/registry/telegram.tools.ts` (регистрация `telegram_search`)
- `packages/core/src/db/schema.ts` (таблица `tg_messages` + FTS, если ещё нет)
...
3. Интеграция в `packages/server/src/index.ts` + env.
