...
- `tasks` + `scheduler_state` таблицы (migration 6) в [packages/core/src/db/schema.ts](../../../packages/core/src/db/schema.ts).
- TasksTable с upsertBySource (3-query tx, idempotent на terminal) в [packages/core/src/db/tables/tasks.ts](../../../packages/core/src/db/tables/tasks.ts).
- SchedulerStateTable с `tryAcquireLock` (single-statement CAS через INSERT OR IGNORE + UPDATE...RETURNING) + `heartbeat` в [packages/core/src/db/tables/scheduler-state.ts](../../../packages/core/src/db/tables/scheduler-state.ts) — готов к использованию в Phase 4.
- MCP `task_add/list/update/start/done/cancel` (scope=agent-only) в [packages/agent/src/mcp/registry/tasks.tools.ts](../../../packages/agent/src/mcp/registry/tasks.tools.ts).
- Domain `TasksTools` в [packages/agent/src/mcp/tools/tasks-tools.ts](../../../packages/agent/src/mcp/tools/tasks-tools.ts).
- REST `/v1/tasks` + `/history` в [packages/server/src/routes/tasks.ts](../../../packages/server/src/routes/tasks.ts).
- Tool-runner: `task_*` timeout 3000ms в [packages/agent/src/pipeline/agent-loop/tool-runner.ts](../../../packages/agent/src/pipeline/agent-loop/tool-runner.ts).
- System-prompt injection (`renderActiveTasks`, `renderTgStatus`) в [packages/agent/src/pipeline/agent-loop/prompt-blocks/tasks.ts](../../../packages/agent/src/pipeline/agent-loop/prompt-blocks/tasks.ts) + подключено в [packages/agent/src/pipeline/agent-loop/system-prompt.ts](../../../packages/agent/src/pipeline/agent-loop/system-prompt.ts).
- `AUTONOMOUS_TASK` env fail-fast в [packages/server/src/app/deps.ts](../../../packages/server/src/app/deps.ts).
...
- HTTP: `fetchJson`/`fetchStream` из `packages/core/src/lib/http-client.ts`.
