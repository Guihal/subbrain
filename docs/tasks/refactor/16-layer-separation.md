...
1. **Auth дырявый.** `/api/token`, `/night-cycle`, `/night-cycle/status` объявлены ДО `authMiddleware` в [src/app/bootstrap.ts:93-103](../../../src/app/bootstrap.ts#L93-L103). Утекает token в ответ. Плюс [src/lib/auth.ts:28](../../../src/lib/auth.ts#L28) глобально пропускает `/telegram/*` — админ-ручки `set-webhook`/`remove-webhook` без защиты.
2. **`tg_send_message` лжёт.** [src/telegram/bot.ts:238](../../../src/telegram/bot.ts#L238) `notify()` глотает err; [src/mcp/executor.ts:76](../../../src/mcp/executor.ts#L76) `tgSendMessage()` не видит падения и всегда возвращает `success:true`. Агент думает «уведомил».
3. **Layer 4 почти пустой.** [src/db/schema.ts:305](../../../src/db/schema.ts#L305) CHECK `role IN (user/assistant/system/tool/reasoning)`; [src/lib/logger.ts:75](../../../src/lib/logger.ts#L75) пишет `_log_${level}` → CHECK violation. [src/lib/logger.ts:78](../../../src/lib/logger.ts#L78) проглатывает exception. Observability иллюзорна. Плюс [src/telegram/userbot.ts:319](../../../src/telegram/userbot.ts#L319) пишет role=`channel_message` — та же история.
4. **Sandbox — не security boundary** (сам [src/pipeline/agent-loop/code-tools/sandbox.ts:10](../../../src/pipeline/agent-loop/code-tools/sandbox.ts#L10) признаёт). Но [src/pipeline/agent-loop/system-prompt.ts:167](../../../src/pipeline/agent-loop/system-prompt.ts#L167) поощряет scheduled-агентов создавать code tools без approval.
...
6. **Cancellation половинчатая.** [src/pipeline/agent-loop/tool-runner.ts:45](../../../src/pipeline/agent-loop/tool-runner.ts#L45) + [src/pipeline/arbitration-room.ts:85](../../../src/pipeline/arbitration-room.ts#L85) делают `Promise.race` без `controller.abort()` — underlying fetch/browser work продолжается в фоне.
7. **Provider startup над-требовательный.** Все virtual roles в [src/lib/model-map.ts:21](../../../src/lib/model-map.ts#L21) primary=MiniMax, но [src/providers/index.ts:23](../../../src/providers/index.ts#L23) всё равно требует Copilot+OpenRouter. `directMode` в [src/routes/chat.ts:31](../../../src/routes/chat.ts#L31) срабатывает по NVIDIA overload даже если модель MiniMax → внезапно обходит pipeline + память.
8. **Shared RAG slaby.** [src/rag/pipeline.ts:156](../../../src/rag/pipeline.ts#L156) vec-путь не подтягивает shared row — snippet пустой. [src/pipeline/agent-pipeline/post/extractors.ts:29](../../../src/pipeline/agent-pipeline/post/extractors.ts#L29) `writeShared` не эмбеддит → semantic-поиск по shared не работает.
