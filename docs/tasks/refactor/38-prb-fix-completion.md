...
grep -c 'original_category' src/services/memory/archive-restore.ts        # expect: >=1
grep -c 'buildEmbeddingMap' src/pipeline/night-cycle/janitor/phase-b-embed.ts  # expect: >=1
test -f src/pipeline/night-cycle/janitor/phase-b-embed.ts && echo OK      # expect: OK
```
