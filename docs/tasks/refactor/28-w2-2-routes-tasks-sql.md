...
Убрать raw SQL (2 запроса в `buildHistoryLoader`) из `packages/server/packages/server/packages/server/src/routes/tasks.ts` + ужать файл с 228 до ≤150 LOC. SoC: routes — view layer.

## Файлы

**Изменить**:
- `packages/server/packages/server/packages/server/src/routes/tasks.ts` (228 → ≤150) — оставить только Elysia route definitions + TypeBox схемы + 2 helper response функции (`notFound`, `badTransition`). `buildHistoryLoader` + `DigestRow` уехал в repo/service. Никаких `memory.db.query(...)`.
- `packages/core/packages/core/packages/core/src/db/tables/tasks.ts` (260 LOC, в whitelist) — добавить 2 метода:
...
**Создать**:
- `packages/core/packages/core/packages/core/src/repositories/task.repo.ts` — пробросить методы tasks из `db/tables/tasks.ts` (включая 2 новых) + `buildHistoryLoader(scope, sinceUnix)` (page-loader для `paginate()`). 

**Сохранить**:
- `packages/agent/src/services/*` — не трогать (если task service отсутствует — route → repo допустим).
- `packages/core/packages/core/packages/core/src/db/index.ts` facade — тех методов, что route уже использует (`listTasks`, `insertTask`, `getTask`, `updateTask`, `transitionTask`, `deleteTask`, `transaction`, `listCompletedTasksSince`).

**Trigger**: `tests/repo-rules.test.ts` `TRANSITIONAL_SQL_ROUTES` Set — удалить `"packages/server/packages/server/src/routes/tasks.ts"`.
**Trigger**: `scripts/check-file-size.ts` `"packages/server/packages/server/src/routes/tasks.ts": 229` → удалить.
...
2. Создать `packages/core/packages/core/packages/core/src/repositories/task.repo.ts`:
   ```ts
...
3. `packages/server/packages/server/packages/server/src/routes/tasks.ts`:
   - Заменить `tasksRoute(memory: MemoryDB)` → `tasksRoute(repo: TaskRepository)`.
   - Удалить `buildHistoryLoader` + `DigestRow` (уехали в repo).
   - Все `memory.X(...)` → `repo.X(...)`.
   - Целевой LOC ≤150.
4. `packages/server/packages/server/packages/server/src/app/deps.ts` или wherever `tasksRoute` создаётся — инжектировать `new TaskRepository(memory)` вместо raw memory.
5. Удалить `"packages/server/packages/server/src/routes/tasks.ts"` из `TRANSITIONAL_SQL_ROUTES` Set + `TRANSITIONAL_WHITELIST` row.
...
6. `grep -nE "SELECT|INSERT INTO|UPDATE.*SET|DELETE FROM" packages/server/packages/server/src/routes/tasks.ts` → пусто.
7. `wc -l packages/server/packages/server/src/routes/tasks.ts` ≤150.
