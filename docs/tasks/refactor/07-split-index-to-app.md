# Задача 07 — Splitting `packages/server/packages/server/packages/server/src/index.ts` → `packages/server/src/app/*`
...
[packages/server/packages/server/src/index.ts](../../../packages/server/packages/server/src/index.ts) разросся до ~440 строк: bootstrap Elysia, оба scheduler'а, autonomous-run SSE-loop, graceful shutdown — всё в одном файле. Разнести по `packages/server/src/app/`, `packages/server/packages/server/packages/server/src/index.ts` оставить ~50 строк.
...
packages/server/packages/server/src/index.ts            # ~50 строк: import {createApp, registerSchedulers, registerShutdown}; start
```
...
- `runAutonomousSSE(req, deps): Response` — извлечь inline-логику из `packages/server/packages/server/packages/server/src/index.ts` (`: ping\n\n` каждые 5 сек, обработка abort).
- Используется и из роута, и из scheduler'а — общий entrypoint.

### `packages/server/packages/server/packages/server/src/app/shutdown.ts`
...
- Импорты из других модулей (`routes/*`, `pipeline/*`) в `packages/server/packages/server/packages/server/src/index.ts` сейчас могут содержать боковые эффекты — после переноса проверить порядок инициализации (`db.migrate()` обязан случиться **до** mount роутов).
...
- [packages/server/packages/server/src/index.ts](../../../packages/server/packages/server/src/index.ts) (сильно сократить)
- `packages/server/packages/server/packages/server/src/app/bootstrap.ts` (новый)
- `packages/server/packages/server/packages/server/src/app/schedulers.ts` (новый)
- `packages/server/src/app/autonomous-run.ts` (новый)
- `packages/server/packages/server/packages/server/src/app/shutdown.ts` (новый)
...
- [ ] `bun run packages/server/packages/server/src/index.ts` стартует, `/health` 200.
- [ ] `bun run tests/integration.live.ts` end-to-end проходит.
- [ ] `wc -l packages/server/packages/server/src/index.ts` ≤ 60.
