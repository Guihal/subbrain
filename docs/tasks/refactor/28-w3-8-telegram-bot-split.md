...
Разбить `src/telegram/bot.ts` (343 LOC) на split-folder. Public API = `TelegramBot` class. **`notify(chatId, msg)` — public logic-API** (см. SoC §3 cross-layer rule); используется scheduler/pipeline для уведомлений.
...
**Создать**:
- `src/telegram/bot/index.ts` — `TelegramBot` class (≤120 LOC). Конструктор + thin делегации.
- `src/telegram/bot/commands.ts` — command handlers (/start, /help, /status, etc — список из текущего bot.ts).
- `src/telegram/bot/routing.ts` — message dispatch (text → AgentLoop, command → command-handler).
- `src/telegram/bot/notify.ts` — **public** `notify(chatId, msg, options?)` — вызывается из scheduler/pipeline для уведомлений. Это logic-helper, НЕ transport.

**Trigger**: `scripts/check-file-size.ts` `"src/telegram/bot.ts": 344` → удалить.
