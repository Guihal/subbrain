...
[packages/agent/src/pipeline/agent-loop/index.ts](../../../packages/agent/src/pipeline/agent-loop/index.ts) (~625 строк) совмещает оркестрацию, шаг, диспатч tool_calls, SSE-heartbeat, hook компрессора и обвязку. Разнести по одной ответственности на файл, сам `index.ts` оставить ≤80 строк фасадом.

## Целевая структура

Внутри той же папки `packages/agent/src/pipeline/agent-loop/`:
...
- [packages/agent/src/pipeline/agent-loop/index.ts](../../../packages/agent/src/pipeline/agent-loop/index.ts) (сократить)
- `packages/agent/src/pipeline/agent-loop/run.ts` (новый)
- `packages/agent/src/pipeline/agent-loop/step.ts` (новый)
- `packages/agent/src/pipeline/agent-loop/tool-dispatch.ts` (новый)
- `packages/agent/src/pipeline/agent-loop/heartbeat.ts` (новый)
- `packages/agent/src/pipeline/agent-loop/compressor-hook.ts` (новый)
...
- [ ] `bunx tsc --noEmit` = 0.
- [ ] `bun test` зелёные (≥80).
- [ ] `wc -l packages/agent/src/pipeline/agent-loop/index.ts` ≤ 80.
