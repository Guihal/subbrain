...
Разбить `src/rag/pipeline.ts` (699 LOC, transitional cap 700) на split-folder. Public API = `RAGPipeline` class + helpers. Whitelist: `rag/pipeline/index.ts:200`. **Bench invariants**: `scripts/bench-rag.ts` (новый, сохраняется в репо).
...
**Создать**:
- `src/rag/pipeline/index.ts` — `RAGPipeline` class (≤200 LOC, whitelist). Конструктор + публичные методы `embedContent`, `indexEntry`, `search`, `getRerankCallsPerSearch` (если есть metric).
- `src/rag/pipeline/forgetting.ts` — Ebbinghaus-like forgetting curve / time-decay scoring (если есть в текущем файле).
- `src/rag/pipeline/boost-persona.ts` — persona-grade boost (+10% rerank weight для kind='persona').
- `src/rag/pipeline/boost-salience.ts` — salience boost (по confidence / age).
- `src/rag/pipeline/rrf.ts` — Reciprocal Rank Fusion для FTS + vec scores.
- `src/rag/pipeline/dedupe.ts` — dedup helpers (semantic + textual).
- `src/rag/pipeline/rerank-call.ts` — NVIDIA NIM rerank вызов + counter (`rerank_calls_per_search` metric).
...
**Trigger**: `scripts/check-file-size.ts` row `"src/rag/pipeline.ts": 700` → удалить. Заменить на `"src/rag/pipeline/index.ts": 200` (CANONICAL_WHITELIST уже содержит).
