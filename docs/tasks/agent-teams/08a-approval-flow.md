...
> **PATH NOTE (2026-05-05):** A2 is not yet merged; `packages/agent/` does not exist. All `packages/agent/*` paths in this doc are **aspirational** — they resolve to `src/pipeline/agent-loop/*` in the current codebase. Kimi MUST create files under `src/pipeline/agent-loop/` (not `packages/agent/`) until A2 merges. The hook dispatcher lives at `src/pipeline/agent-loop/tool-runner.ts` today; 8a-3 registers its gate there.
...
Independent of Phase 5 observability — `metrics_log` table already exists (`src/db/schema.ts:218`).
...
> **ToolResult shape (current codebase):** `{success: boolean, data?: unknown, error?: {code, message} | string}` (see `src/mcp/types.ts`). The doc uses `error.code` notation — this means `error` is the structured `{code, message}` form. If A2-5 (5-variant `kind` union) merges before 8a, the shape may change; 8a code must adapt to whichever shape is live at merge time.
...
`src/pipeline/agent-loop/plugins-internal.ts` (A2-9 already pins
...
- No re-implementation of A2 hooks. 8a-3 must register through the hook dispatcher (`src/pipeline/agent-loop/tool-runner.ts` today, `packages/agent/hooks/dispatcher.ts` after A2); if A2-3/A2-4/A2-5 are not merged, 8a-3 fails fast with `FAIL: dependency_missing: A2-3/A2-4/A2-5`.
...
- **agentMode** — value from `PublicToolContext.agentMode` (`"interactive" | "scheduled" | undefined`). Existing field, see `src/mcp/registry/tool-registry.ts`. Both modes are gated for tools in the seed list.
...
- **ToolResult** — current shape: `{success: boolean, data?: unknown, error?: {code: string, message: string} | string}` (`src/mcp/types.ts`). 8a constructs structured errors as `{success: false, error: {code, message}}`. If A2-5 (5-variant `kind` union) merges before 8a, adapt to the new shape at merge time — do not pre-implement the unmerged union.
- **metrics_log** — existing table (`src/db/schema.ts:218`); 8a-6 reuses it (no new table) for approval audit entries with `snapshot.kind="approval_decision"`.
...
{
  "task_id": "8a-1",
  "goal": "Add approvals table to src/db/schema.ts and export Approval domain types from src/db/tables/approvals.ts.",
  "non_goals": [
  ...
  "allowed_write_paths": [
    "src/db/schema.ts",
    "src/db/tables/approvals.ts"
  ],
  "read_context": [
    "src/db/schema.ts:215-225",
    "src/db/tables/freelance-leads.ts",
  ...
  "acceptance": [
    ...
    "rg -n 'CREATE TABLE IF NOT EXISTS approvals' src/db/schema.ts",
    "rg -n 'export interface Approval' src/db/tables/approvals.ts"
  ],
  "diff_budget_loc": 180,
  "file_count_max": 3,
  ...
  "escalation_triggers": [
    "Spec contradicts existing migration pattern in src/db/schema.ts.",
  ...
  "glossary": {
    ...
    "Approval": "TS interface mirroring the row shape; exported from src/db/tables/approvals.ts.",
...
{
  "task_id": "8a-2",
  "goal": "Add src/mcp/registry/approval-registry.ts exposing requiresApproval(toolName, agentMode), the gated-tool seed list with BOTH tg_send_message and tg_send_report in BOTH agentMode values, canonicalizeArgs() helper, and resolveOperatorChat() reading APPROVAL_OPERATOR_CHAT_ID with TG_OWNER_CHAT_ID fallback.",
  "non_goals": [
  ...
  "allowed_write_paths": [
    "src/mcp/registry/approval-registry.ts",
    "tests/approval-registry.test.ts"
  ],
  "read_context": [
    "src/mcp/registry/telegram-spam-gate.ts",
    "src/mcp/registry/tool-registry.ts:30-90",
    "src/mcp/registry/telegram.tools.ts:93-118",
    "src/mcp/registry/report.tools.ts:45-62",
    "src/mcp/tools/telegram-report.ts:41-76",
  ...
  "acceptance": [
    ...
    "rg -n 'export function requiresApproval' src/mcp/registry/approval-registry.ts",
    "rg -n 'export function resolveOperatorChat' src/mcp/registry/approval-registry.ts",
    "rg -n 'export function canonicalizeArgs' src/mcp/registry/approval-registry.ts",
    "rg -n \"tool: 'tg_send_message'\" src/mcp/registry/approval-registry.ts",
    "rg -n \"tool: 'tg_send_report'\" src/mcp/registry/approval-registry.ts"
  ],
  "diff_budget_loc": 160,
  "file_count_max": 2,
  "rollback": "Delete src/mcp/registry/approval-registry.ts + test file; nothing imports it yet so deletion is safe.",
  "escalation_triggers": [
    ...
    "Audit of src/mcp/registry/*.ts surfaces a third tool that calls executor.tgSendMessage — escalate; gated set must cover all such tools."
  ],
  "glossary": {
...
{
  "task_id": "8a-3",
  "goal": "Add src/pipeline/agent-loop/approval-gate/index.ts as a Plugin that registers a tool.execute.before hook checking requiresApproval, calling resolveOperatorChat for availability, and either short-circuiting with {success:false, error:{code:'awaiting_approval'|'approval_unavailable'|'approval_denied'|'approval_expired'}} or allowing through to the handler when a fresh approved row exists; append the plugin to INTERNAL_PLUGINS in src/pipeline/agent-loop/plugins-internal.ts AFTER tg-gates, code-tool-guards, scheduled-blacklist so auto-deny still fires first.",
  "non_goals": [
    ...
    "Use current ToolResult shape {success, error?: {code,message}|string} from src/mcp/types.ts. If A2-5 (5-variant kind union) merges before 8a, adapt at merge time — do not pre-implement unmerged shapes."
  ],
  "allowed_write_paths": [
    "src/pipeline/agent-loop/approval-gate/index.ts",
    "src/pipeline/agent-loop/approval-gate/lookup.ts",
    "src/pipeline/agent-loop/plugins-internal.ts",
    "src/db/tables/approvals.ts",
  ...
  "acceptance": [
    ...
    "rg -n 'awaiting_approval' src/pipeline/agent-loop/approval-gate/index.ts",
    "rg -n 'approval_unavailable' src/pipeline/agent-loop/approval-gate/index.ts",
    "rg -n 'requiresApproval' src/pipeline/agent-loop/approval-gate/index.ts",
    "rg -n 'success: false' src/pipeline/agent-loop/approval-gate/index.ts",
    "rg -n 'approval-gate' src/pipeline/agent-loop/plugins-internal.ts",
    "node -e 'const s=require(\"fs\").readFileSync(\"src/pipeline/agent-loop/plugins-internal.ts\",\"utf8\"); const order=[\"code-tool-guards\",\"tg-gates\",\"scheduled-blacklist\",\"freelance-scout\",\"approval-gate\"]; let last=-1; for (const n of order){const i=s.indexOf(n); if(i<=last){console.error(\"order broken at\",n);process.exit(1);} last=i;}'"
  ],
  "diff_budget_loc": 280,
  "file_count_max": 5,
  "rollback": "Revert src/pipeline/agent-loop/plugins-internal.ts + delete approval-gate plugin dir + revert approvals.ts helpers; auto-deny plugins keep working unchanged.",
  "escalation_triggers": [
    ...
    "A2-3 / A2-4 / A2-5 not merged AND no interim hook point in tool-runner.ts — return FAIL: dependency_missing immediately, do not write any file. The current tool-runner.ts (src/pipeline/agent-loop/tool-runner.ts) has no before-hook dispatcher; 8a-3 MUST have a hook point.",
...
```json
{
  ...
  "read_context": [
    ...
    "src/telegram/bot/commands.ts",
    "src/app/deps.ts:338-367",
    "src/db/tables/approvals.ts",
    "src/mcp/registry/approval-registry.ts",
    "src/pipeline/agent-loop/approval-gate/index.ts"
  ],
  ...
  "acceptance": [
    ...
    "rg -n 'approve:|deny:' src/telegram/bot/approvals.ts",
    "rg -n 'resolveOperatorChat' src/telegram/bot/approvals.ts"
  ],
  "diff_budget_loc": 260,
  "file_count_max": 4,
...
```json
{
  ...
  "allowed_write_paths": [
    "src/scheduler/approval-sweeper.ts",
  ...
  "read_context": [
    ...
    "src/db/tables/approvals.ts",
    "src/pipeline/agent-loop/approval-gate/index.ts",
    "src/pipeline/agent-loop/run.ts:42-97"
  ],
  ...
  "acceptance": [
    ...
    "rg -n 'expires_at < unixepoch' src/scheduler/approval-sweeper.ts",
    "rg -n 'installApprovalSweeper' src/app/schedulers.ts",
    "rg -n 'retry-on-next-invocation\\|next agent invocation' src/scheduler/approval-sweeper.ts"
  ],
  "diff_budget_loc": 160,
  "file_count_max": 3,
...
```json
{
  ...
  "read_context": [
    "src/lib/metrics.ts:80-110",
    "src/db/schema.ts:218-225",
    "src/db/tables/approvals.ts"
  ],
  "risk_tier": "public-api",
  "acceptance": [
    ...
    "rg -n 'logApprovalDecision' src/pipeline/agent-loop/approval-gate/index.ts src/telegram/bot/approvals.ts src/scheduler/approval-sweeper.ts"
  ],
  "diff_budget_loc": 180,
  "file_count_max": 4,
  ...
  "glossary": {
    "snapshot.kind": "Discriminator string. Existing values live in src/lib/metrics.ts; this packet adds 'approval_decision'.",
...
```json
{
  ...
  "non_goals": [
    ...
    "Use current ToolResult shape {success, error?} from src/mcp/types.ts in tests. If A2-5 merges before 8a, adapt test assertions to the new shape at merge time."
  ],
  "allowed_write_paths": [
  ...
  "escalation_triggers": [
    ...
    "Stubbing grammy bot requires undocumented internal — switch to direct call into src/telegram/bot/approvals.ts handler with synthetic callback payload.",
