...
```json
{
  ...
  "non_goals": [
    ...
    "Do not modify any other migration function in src/db/schema.ts.",
    "Do not touch src/scheduler/, src/pipeline/, src/mcp/, src/services/ in this packet.",
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/db/tables/agent-tasks.ts)\" -le 150",
    "test \"$(wc -l < src/repositories/agent-tasks.repo.ts)\" -le 150",
    "test \"$(wc -l < src/db/tables/agent-tasks/types.ts)\" -le 80",
    ...
    "test -z \"$(grep -rnE 'agent_tasks|agentTasksRepo' src/services/ src/pipeline/ src/scheduler/ src/routes/ 2>/dev/null)\""
  ],
  "diff_budget_loc": 280,
  "file_count_max": 7,
  ...
  "glossary": {
    ...
    "AgentTaskRecord": "TypeScript row interface defined in src/db/tables/agent-tasks/types.ts per PRD 39 lines 124-137.",
    "claimNext": "Atomic UPDATE...RETURNING that flips one pending row to running, see PRD 39 lines 178-189.",
    "MemoryDB": "Existing facade class in src/db/index.ts, this packet only adds a getter agentTasksRepo.",
    "migration_19": "Per-version migration function added to src/db/schema.ts following the migration_10..migration_18 pattern in that same file. P2-1 owns mig19; collision avoidance is recorded in docs/specs/wave-plan-2026-05.md:84-92 (P3-2=17, P3-5=18, P2-1=19)."
  }
}
```
...
```json
{
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/routes/agent-tasks.ts)\" -le 150",
    "grep -nE 'paginate\\(|PaginatedResponse' src/routes/agent-tasks.ts",
    "grep -nE 'authMiddleware' src/routes/agent-tasks.ts",
    "test -z \"$(grep -nE 'SELECT|INSERT INTO|UPDATE .* SET|DELETE FROM' src/routes/agent-tasks.ts)\""
  ],
  "diff_budget_loc": 220,
  "file_count_max": 3,
  ...
  "glossary": {
    "PaginatedResponse": "Envelope type {items, total} from src/lib/api-envelope.ts, used by routes/memory.ts and routes/freelance.ts.",
    "authMiddleware": "Bearer-token middleware exported from src/lib/auth.ts (Elysia plugin); every route in this packet must be behind it.",
    "Elysia TypeBox": "t.Object/t.Literal/t.Union schemas used for body and query validation; see src/routes/memory.ts for the canonical pattern.",
...
```json
{
  ...
  "non_goals": [
    ...
    "Do not modify src/scheduler/free-agent.ts in this packet (legacy bridge handled in P2-5)."
  ],
  "allowed_write_paths": [
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/scheduler/agent-pool/index.ts)\" -le 100",
    "test \"$(wc -l < src/scheduler/agent-pool/pool/index.ts)\" -le 100",
    "test \"$(wc -l < src/scheduler/agent-pool/types.ts)\" -le 80",
    "test -z \"$(grep -rnE 'Promise\\.all\\b' src/scheduler/agent-pool/)\"",
    "test -z \"$(grep -rnE 'as any' src/scheduler/agent-pool/)\"",
    "test -z \"$(grep -rnE '\\bfetch\\(' src/scheduler/agent-pool/)\""
  ],
  "diff_budget_loc": 250,
  "file_count_max": 4,
  ...
  "escalation_triggers": [
    ...
    "`Promise.all(` (without `Settled`) appears anywhere under `src/scheduler/agent-pool/` — return `FAIL: promise-all-banned` per CLAUDE.md §2."
  ],
  "glossary": {
    "AgentTaskPool": "Thin facade class in src/scheduler/agent-pool/pool/index.ts that wraps agentTasksRepo with claim/complete/noop/fail/enqueue/markZombiesFailed; no business logic.",
    ...
    "RunnerResult": "Discriminated union {status:'complete', artifact} | {status:'noop', reason} | {status:'failed', reason} defined in src/scheduler/agent-pool/types.ts.",
...
{
  "task_id": "P2-4",
  "goal": "Register `done_with_artifact` agent-only MCP tool that validates status/artifact/reason and signals AgentLoop termination via the same control-signal pattern as the existing inline `done` tool (in `src/mcp/registry/agent-meta.tools.ts:28`). The handler returns `{ok:true, data:<JSON-string-or-structured>}`; agent-loop's existing terminate dispatch reads that `data` to end the loop. ToolResult shape is **unchanged**.",
  "non_goals": [
    "Do not remove or rename the existing inline `done` tool registered in src/mcp/registry/agent-meta.tools.ts:28.",
  ...
  "read_context": [
    ...
    "src/mcp/registry/index.ts",
    "src/mcp/registry/agent-meta.tools.ts",
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/mcp/registry/pool.tools.ts)\" -le 120",
    "test \"$(wc -l < src/mcp/tools/pool/done-with-artifact.ts)\" -le 80",
    "grep -nE 'scope:\\s*\"agent-only\"' src/mcp/registry/pool.tools.ts",
    "grep -nE 'done_with_artifact' src/mcp/registry/pool.tools.ts",
    "grep -nE 'registerAgentMetaTools' src/mcp/registry/agent-meta.tools.ts",
    "test -z \"$(grep -nE 'export const ToolResult|interface ToolResult' src/mcp/types.ts | grep -v 'ok:\\s*true|ok:\\s*false')\""
  ],
  "diff_budget_loc": 200,
  "file_count_max": 4,
  ...
  "escalation_triggers": [
    ...
    "Worker cannot find `src/mcp/tools/done-tool.ts` — that file does NOT exist; the existing `done` tool is inline at `src/mcp/registry/agent-meta.tools.ts:28`. Read that file instead, do not invent a path."
  ],
  "glossary": {
    ...
    "ToolResult": "Discriminated union {success:true, data} | {success:false, error:{code, message}|string} from src/mcp/types.ts. UNCHANGED by this packet — terminate is encoded inside `data`.",
    ...
    "terminate (control signal)": "Agent-loop already interprets the existing `done` tool's `data` string as a terminate signal (see src/pipeline/agent-loop/tool-dispatch.ts and agent-loop/index.ts). `done_with_artifact` extends that pattern: handler returns `{ok:true, data}` where `data` is either a string (legacy) or a JSON-stringified `{terminate:true, status, artifact?, reason?}`. Agent-loop parses both. NO new ToolResult field is introduced.",
...
```json
{
  ...
  "non_goals": [
    ...
    "Do not edit src/scheduler/, src/services/, src/routes/, src/mcp/."
  ],
  "allowed_write_paths": [
    "src/pipeline/agent-loop/types.ts",
    "src/pipeline/agent-loop/run.ts",
    "src/pipeline/agent-loop/stream.ts",
  ...
  "acceptance": [
    ...
    "bun test",
    "grep -E 'onUsage|signal|systemMessage|userMessage' src/pipeline/agent-loop/types.ts",
    "grep -nE 'onUsage|signal' src/pipeline/agent-loop/run.ts",
    "grep -nE 'systemMessage|userMessage' src/pipeline/agent-loop/run.ts",
    "test -z \"$(grep -rnE 'as any' src/pipeline/agent-loop/run.ts src/pipeline/agent-loop/stream.ts src/pipeline/agent-loop/step.ts src/pipeline/agent-loop/types.ts)\""
  ],
  "diff_budget_loc": 180,
  "file_count_max": 5,
  ...
  "escalation_triggers": [
    ...
    "Adding `signal` requires changing the underlying provider signature in src/providers/* beyond what `signal` already supports — return `FAIL: scope: provider signal threading needs separate PR`.",
  ...
  "glossary": {
    "AgentLoopRequest": "Public input type at src/pipeline/agent-loop/types.ts. Currently `{task, model?, maxSteps?, sessionId?, priority?, schedule?, agentMode?, agentId?}` (verified 2026-05-05). This packet adds four optional fields — no other change.",
...
```json
{
  ...
  "non_goals": [
    ...
    "Do not delete src/scheduler/free-agent.ts — bridge mode only.",
  ...
  "read_context": [
    ...
    "src/pipeline/agent-loop/code-tools/telegram-spam-gate.ts",
    "src/scheduler/free-agent.ts"
  ],
  "risk_tier": "ordinary",
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/scheduler/agent-pool/runners/free.ts)\" -le 150",
    "test \"$(wc -l < src/scheduler/agent-pool/index.ts)\" -le 100",
    "grep -niE 'save tokens|be efficient|постарайся уложиться|не используй tool без нужды' src/scheduler/agent-pool/runners/free.ts | wc -l | grep -q '^0$'",
    "grep -nE 'AGENT_POOL_MAX_TOKENS_FREE' .env.example",
    "grep -nE 'legacy-free-agent' src/scheduler/free-agent.ts"
  ],
  "diff_budget_loc": 290,
  "file_count_max": 5,
  "rollback": "Revert the commit; pool falls back to default no-op runFn from P2-3, free-agent.ts loses bridge.",
  "escalation_triggers": [
    "P2-5a not merged — `grep -E 'onUsage|signal|systemMessage|userMessage' src/pipeline/agent-loop/types.ts` returns nothing — return `FAIL: pre-req-missing: P2-5a not merged (AgentLoopRequest lacks onUsage/signal/systemMessage/userMessage)`.",
  ...
  "glossary": {
    "runFreeTask": "Async function in src/scheduler/agent-pool/runners/free.ts that calls AgentLoop.run with model 'teamlead', priority 'low', agentMode 'scheduled', maxSteps 50, signal from token-budget AbortController, and parses the final tool result into RunnerResult (PRD 40 lines 168-188).",
...
```json
{
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/scheduler/agent-pool/pool/rate-limits.ts)\" -le 80",
    "test \"$(wc -l < src/scheduler/agent-pool/digest.ts)\" -le 100",
    "test \"$(wc -l < src/scheduler/agent-pool/index.ts)\" -le 100",
    "grep -nE 'AGENT_POOL_DIGEST_HOUR_LOCAL' .env.example",
    "test -z \"$(grep -rnE 'Promise\\.all\\b' src/scheduler/agent-pool/)\"",
    "test -z \"$(grep -rnE 'INSERT INTO|UPDATE .* SET' src/scheduler/agent-pool/digest.ts src/scheduler/agent-pool/pool/rate-limits.ts)\"",
    "( ! grep -rnE 'for\\s*\\([^)]*\\)\\s*\\{[^}]*\\.(insert|update|delete)\\(' src/scheduler/agent-pool/ ) || grep -rnE 'db\\.transaction\\(' src/scheduler/agent-pool/"
  ],
  "diff_budget_loc": 270,
  "file_count_max": 6,
...
```json
{
  ...
  "acceptance": [
    ...
    "test -z \"$(grep -rnE 'src/lib/mutex' src/scheduler/ src/services/ src/routes/ 2>/dev/null)\""
  ],
  "diff_budget_loc": 120,
  "file_count_max": 2,
...
```json
{
  ...
  "non_goals": [
    ...
    "Do not edit src/pipeline/, src/services/, src/routes/.",
  ...
  "acceptance": [
    ...
    "bun test",
    "test \"$(wc -l < src/scheduler/agent-pool/pool/concurrency.ts)\" -le 100",
    "test \"$(wc -l < src/scheduler/agent-pool/index.ts)\" -le 100",
    "grep -cE 'Promise\\.allSettled' src/scheduler/agent-pool/index.ts | grep -qvE '^0$'",
    "test 0 -eq \"$(grep -cE 'Promise\\.all\\b' src/scheduler/agent-pool/index.ts)\"",
    "grep -nE 'RETURNING \\*' src/db/tables/agent-tasks.ts",
    "grep -nE \"WHERE id=\\? AND status='pending'\" src/db/tables/agent-tasks.ts",
