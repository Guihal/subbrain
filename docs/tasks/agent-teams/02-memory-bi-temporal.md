...
2. `docs/tasks/memory-v2/M-12-archive-confidence-real.md` — archive
   ...
   Migration 15 at `src/db/schema.ts:820-862`). Phase 3 keeps a verification
   packet only.
3. `docs/tasks/refactor/43-prd-hippocampus-rewrite.md` — PR-D hippocampus
   rewrite + write-cap (includes `src/lib/metrics.ts` counter scope).
...
5. Do NOT change embed model / rerank model selection — `EMBED_MODEL` /
   `RERANK_MODEL` in `src/lib/model-map.ts` are fixed.
...
(`src/db/schema.ts`). They CANNOT run in parallel — both would claim version
...
2. **P3-3 layer scope:** filter is applied to BOTH `shared_memory` and
   ...
   retrieval is currently in `src/db/tables/shared/search-vec.ts`; context
   retrieval in `src/db/tables/memory/*` and `src/rag/pipeline/{fts,vec}.ts`.
...
5. **P3-6 NIGHT_CYCLE_MODEL fallback:** must be centralised. Five sites
   ...
   `src/pipeline/night-cycle/steps/shared.ts:8`,
   `src/pipeline/night-cycle/prune/shared.ts:7`,
   `src/pipeline/night-cycle/prune/context.ts:8`,
   `src/pipeline/night-cycle/prune/focus.ts:7`,
   `src/pipeline/night-cycle/prune/tasks-classify.ts:102`. P3-6 introduces
   `src/pipeline/night-cycle/model.ts` (single export `resolveNightModel()`)
...
Earlier drafts referenced `src/rag/pipeline.ts`. That file does NOT exist.
Real layout is split: `src/rag/pipeline/index.ts` plus
...
```json
{
  ...
  "non_goals": [
    "Do not modify `src/db/schema.ts` Migration 15 SQL.",
  ...
  "acceptance": [
    ...
    "test \"$(grep -c 'PRAGMA user_version = 15' src/db/schema.ts)\" -ge 1",
  ...
  "escalation_triggers": [
    "Migration 15 missing from src/db/schema.ts",
  ...
  "glossary": {
    "Migration 15": "src/db/schema.ts block guarded by `if (version < 15)` that rebuilds layer3_archive with REAL confidence",
...
> `src/db/schema.ts` (currently `user_version=16` at line 879). Migration
...
```json
{
  ...
  "allowed_write_paths": [
    "src/db/schema.ts",
    "src/db/types.ts",
    "src/db/tables/memory/helpers.ts",
    "src/db/tables/memory/context.ts",
    "src/db/tables/shared/helpers.ts",
    "src/db/tables/shared/index.ts",
    "src/repositories/memory/index.ts",
    "tests/memory-bi-temporal-schema.test.ts"
  ],
  "read_context": [
    "src/db/schema.ts:860-887",
    "src/db/tables/memory/context.ts",
    "src/db/tables/memory/helpers.ts",
    "src/db/tables/shared/helpers.ts",
    "src/db/tables/shared/index.ts",
    "src/db/tables/shared/search-vec.ts",
    "src/repositories/memory/index.ts",
    "src/db/types.ts"
  ],
  "risk_tier": "schema",
  ...
  "diff_budget_loc": 320,
  "file_count_max": 8,
  "rollback": "Drop the `if (version < 17)` block in src/db/schema.ts; `valid_from`/`valid_to`/`observed_at` are additive nullable columns so no data loss. Revert helper additions in shared/memory tables.",
  "escalation_triggers": [
  ...
  "glossary": {
    ...
    "shared CRUD path": "src/db/tables/shared/helpers.ts (SHARED_UPDATABLE allow-list) + shared/index.ts (insertShared/updateShared) + repositories/memory/index.ts facade — bi-temporal columns must round-trip through ALL three"
  }
}
```
...
```json
{
  ...
  "allowed_write_paths": [
    "src/db/tables/memory/helpers.ts",
    "src/db/tables/memory/search.ts",
    "src/db/tables/shared/helpers.ts",
    "src/db/tables/shared/search-vec.ts",
    "src/rag/pipeline/fts.ts",
    "src/rag/pipeline/vec.ts",
  ...
  "acceptance": [
    ...
    "bun test 2>&1 | tail -3 | grep -q '0 fail'",
    "grep -nE 'AND \\(valid_from IS NULL OR valid_from <= unixepoch\\(\\)\\) AND \\(valid_to IS NULL OR valid_to > unixepoch\\(\\)\\)' src/db/tables/memory/helpers.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'AND \\(valid_from IS NULL OR valid_from <= unixepoch\\(\\)\\) AND \\(valid_to IS NULL OR valid_to > unixepoch\\(\\)\\)' src/db/tables/shared/helpers.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'valid_to IS NULL OR valid_to' src/rag/pipeline/fts.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'valid_to IS NULL OR valid_to' src/rag/pipeline/vec.ts | wc -l | grep -qE '^[1-9]'",
    "bun run scripts/check-file-size.ts",
    "bun run scripts/check-deep-imports.ts"
  ],
  "diff_budget_loc": 260,
  ...
  "glossary": {
    ...
    "buildActiveFilter": "existing helper duplicated in `src/db/tables/{memory,shared}/helpers.ts` that composes status='active' + supersedes/expires clauses; extend BOTH copies with the temporal predicate and add a unit test that mixes status with temporal filter to assert no AND/OR precedence leak",
...
```json
{
  ...
  "read_context": [
    "src/rag/pipeline/boosts.ts",
    "src/rag/pipeline/index.ts",
    "src/db/tables/edges.ts",
    "src/db/index.ts:425-450",
    "src/db/tables/memory/index.ts"
  ],
  "risk_tier": "ordinary",
  "acceptance": [
    "bunx tsc --noEmit",
    ...
    "bun test 2>&1 | tail -3 | grep -q '0 fail'",
    "grep -nE 'applyEdgeWalkBoost|EDGE_WALK_BOOST' src/rag/pipeline/boosts.ts | wc -l | grep -qE '^[2-9]'",
    "grep -nE 'applyEdgeWalkBoost' src/rag/pipeline/index.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'getRelated\\(' src/rag/pipeline/boosts.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'Math\\.max\\(' src/rag/pipeline/boosts.ts | wc -l | grep -qE '^[1-9]'",
    "bun run scripts/check-file-size.ts",
    "bun run scripts/check-deep-imports.ts"
  ],
  "diff_budget_loc": 220,
  "file_count_max": 3,
  "rollback": "Remove `applyEdgeWalkBoost` import + call from `src/rag/pipeline/index.ts`; helper becomes dead code.",
  "escalation_triggers": [
...
> `src/db/schema.ts`. Operator gate-keeps the merge order; Kimi is told to
> abort outright.
...
```json
{
  ...
  "goal": "Add `memory_blocks` table (id TEXT PK, owner_role TEXT, label TEXT, body TEXT NOT NULL, created_at INTEGER, updated_at INTEGER, version INTEGER NOT NULL DEFAULT 1, UNIQUE(owner_role, label)) via Migration 18 with idempotent guard; expose CRUD helpers in new `src/db/tables/memory/blocks.ts`.",
  "non_goals": [
  ...
  "allowed_write_paths": [
    "src/db/schema.ts",
    "src/db/types.ts",
    "src/db/tables/memory/blocks.ts",
    "src/db/tables/memory/index.ts",
  ...
  "glossary": {
    ...
    "owner_role": "TEXT virtual-role id from src/lib/model-map.ts (teamlead|coder|critic|memory|...)",
...
{
  "task_id": "P3-6",
  "goal": "Add `sleep` virtual role in `src/lib/model-map.ts` mapping to `deepseek-ai/deepseek-v4-flash` (NIM, fallback `MiniMax-M2.7`). Centralise the `NIGHT_CYCLE_MODEL` env-fallback resolver in a new `src/pipeline/night-cycle/model.ts` (`resolveNightModel()` returns `process.env.NIGHT_CYCLE_MODEL || \"sleep\"`) and rewire ALL FIVE duplicate sites to use it.",
  "non_goals": [
  ...
  "allowed_write_paths": [
    ...
    "src/pipeline/night-cycle/model.ts",
    "src/pipeline/night-cycle/steps/shared.ts",
    "src/pipeline/night-cycle/prune/shared.ts",
    "src/pipeline/night-cycle/prune/context.ts",
    "src/pipeline/night-cycle/prune/focus.ts",
    "src/pipeline/night-cycle/prune/tasks-classify.ts",
    "tests/model-map-sleep-role.test.ts"
  ],
  "read_context": [
    "src/lib/model-map.ts",
    "src/pipeline/night-cycle/index.ts",
    "src/pipeline/night-cycle/steps/shared.ts",
    "src/pipeline/night-cycle/prune/shared.ts",
    "src/pipeline/night-cycle/prune/context.ts",
    "src/pipeline/night-cycle/prune/focus.ts",
  ...
  "acceptance": [
    ...
    "bun test 2>&1 | tail -3 | grep -q '0 fail'",
    "grep -nE '^\\s*sleep:\\s*\\{' src/lib/model-map.ts | wc -l | grep -qE '^[1-9]'",
    "grep -nE 'export function resolveNightModel' src/pipeline/night-cycle/model.ts | wc -l | grep -qE '^[1-9]'",
    "test \"$(grep -rnE 'process\\.env\\.NIGHT_CYCLE_MODEL' src/pipeline/night-cycle/ | wc -l)\" -eq 1",
    "test \"$(grep -rnE 'NIGHT_CYCLE_MODEL\\s*\\|\\|' src/pipeline/night-cycle/ | wc -l)\" -eq 1",
    "grep -rnE 'resolveNightModel\\(' src/pipeline/night-cycle/ | wc -l | grep -qE '^[5-9]|[1-9][0-9]'",
    "bun run scripts/check-file-size.ts",
    "bun run scripts/check-deep-imports.ts",
  ...
  "glossary": {
    ...
    "five duplicate sites": "src/pipeline/night-cycle/steps/shared.ts:8, prune/shared.ts:7, prune/context.ts:8, prune/focus.ts:7, prune/tasks-classify.ts:102 — all collapse to a single `resolveNightModel()` call",
...
{
  "task_id": "P3-7",
  "goal": "Implement PR-D from `docs/tasks/refactor/43-prd-hippocampus-rewrite.md`: rewrite `getExtractorPrompt` body (text only), add `MAX_WRITES_PER_EXCHANGE=3` cap + telemetry counter (via `src/lib/metrics.ts` — file already exists per source PRD §3) + `logger.info` events in `runHippocampus`, update tests.",
  "non_goals": [
    "Do not change `MAX_HIPPO_STEPS=5`.",
    "Do not change signatures of `getExtractorPrompt`, `runHippocampus`, or `MIN_EXTRACTION_LENGTH`.",
    "Do not touch `src/pipeline/agent-pipeline/post/extractors.ts` or `gate.ts`.",
  ...
  "read_context": [
    ...
    "src/pipeline/agent-pipeline/post/hippocampus.ts",
    "src/pipeline/agent-pipeline/post/prompt.ts",
    "src/pipeline/agent-pipeline/post/extractors.ts:1-30",
    "src/pipeline/agent-pipeline/post/gate.ts:1-30",
    "src/lib/metrics.ts"
  ],
  "risk_tier": "public-api",
  "acceptance": [
    ...
    "bun run scripts/check-file-size.ts",
    "bun run scripts/check-deep-imports.ts",
    "test \"$(wc -l < src/pipeline/agent-pipeline/post/hippocampus.ts)\" -le 150",
    "test \"$(wc -l < src/pipeline/agent-pipeline/post/prompt.ts)\" -le 150",
    "grep -cE 'MAX_WRITES_PER_EXCHANGE\\s*=\\s*3' src/pipeline/agent-pipeline/post/hippocampus.ts | grep -qE '^[1-9]'",
    "grep -cE 'limit_exceeded' src/pipeline/agent-pipeline/post/hippocampus.ts | grep -qE '^[1-9]'",
    "grep -cE 'hippocampus_writes_per_exchange|writes_count' src/lib/metrics.ts | grep -qE '^[1-9]'",
    "grep -cE 'surprising|non-obvious|actionable' src/pipeline/agent-pipeline/post/prompt.ts | grep -qE '^[3-9]|[1-9][0-9]'",
    "test \"$(grep -cniE 'save token|be efficient|постарайся уложиться|не пиши слишком много|не используй tool без нужды' src/pipeline/agent-pipeline/post/prompt.ts)\" -eq 0",
    "grep -cE 'MAX_HIPPO_STEPS\\s*=\\s*5' src/pipeline/agent-pipeline/post/hippocampus.ts | grep -qE '^[1-9]'",
    "grep -cE 'export function getExtractorPrompt\\b' src/pipeline/agent-pipeline/post/prompt.ts | grep -qE '^[1-9]'",
  ...
  "escalation_triggers": [
    ...
    "`src/lib/metrics.ts` shape diverges from existing counters — match the existing pattern, do NOT introduce a new metrics framework"
  ],
  "glossary": {
    ...
    "telemetry counter": "`hippocampus_writes_per_exchange` histogram in `src/lib/metrics.ts` (file already exists; PRD §3 requires it). Plus `logger.info(\"hippocampus\", \"<event>\", { exchange_id, writes_count, skipped_dup_count })` per CLAUDE.md guardrail #9"
  }
}
```
...
```json
{
  ...
  "acceptance": [
    ...
    "bun run scripts/check-file-size.ts",
    "bun run scripts/check-deep-imports.ts",
    "test \"$(wc -l < src/pipeline/arbitration/prompts.ts)\" -le 150",
    "test \"$(wc -l < src/pipeline/agent-pipeline/post/prompt.ts)\" -le 150",
    "grep -cE 'рассудительный тимлид|verification|hedges?' src/pipeline/arbitration/prompts.ts | grep -qE '^[3-9]|[1-9][0-9]'",
    "grep -cE 'гиппокамп|surprising|stenographer|архивист' src/pipeline/agent-pipeline/post/prompt.ts | grep -qE '^[3-9]|[1-9][0-9]'",
    "grep -cE 'memory_search.*candidate|MAX_WRITES_PER_EXCHANGE|whitelist' src/pipeline/agent-pipeline/post/prompt.ts | grep -qE '^[1-9]'",
    "test \"$(grep -cniE 'save token|be efficient|постарайся уложиться|не пиши слишком много|не используй tool без нужды' src/pipeline/arbitration/prompts.ts src/pipeline/agent-pipeline/post/prompt.ts)\" -eq 0",
    "grep -cE 'export function buildSynthesisSystemPrompt\\b' src/pipeline/arbitration/prompts.ts | grep -qE '^[1-9]'",
    "grep -cE 'export function getExtractorPrompt\\b' src/pipeline/agent-pipeline/post/prompt.ts | grep -qE '^[1-9]'",
    "test \"$(git diff --name-only HEAD | grep -cE 'post/(hippocampus|extractors|gate)\\.ts$|arbitration-room\\.ts$')\" -eq 0",
    "test \"$(git diff src/pipeline/arbitration/prompts.ts src/pipeline/agent-pipeline/post/prompt.ts | grep -cE '^\\+(import|export)')\" -eq 0"
  ],
  "diff_budget_loc": 100,
  "file_count_max": 4,
...
```json
{
  ...
  "non_goals": [
    "Do not modify M-10 curation tool source files (`src/mcp/tools/memory-curation/*` or wherever they live).",
  ...
  "escalation_triggers": [
    ...
    "Curation MCP tools not present in `src/mcp/registry` — M-10 may not have landed; STOP",
...
- OpenTelemetry / Langfuse decision is **Phase 5** — telemetry calls in
  P3-7 use existing `src/lib/metrics.ts` + plain `logger.info` per CLAUDE.md
...
- `grep -rnE 'process\\.env\\.NIGHT_CYCLE_MODEL' src/pipeline/night-cycle/ | wc -l` == 1 (single resolver)
