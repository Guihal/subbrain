...
Tests: `tests/post-link-related-evolution.test.ts` (8 cases: empty-tags, pure-add, dup-dedup, cap-eviction, no-op-already-covered, layer-routing-shared, env-disabled, neighbour-deleted-mid-flight). bun test full → 771 pass / 1 fail (pre-existing `usemarkdown.test.ts` module-resolution FP, unchanged baseline) / 1 error. tsc 0. grep `evolveNeighbour|LINK_EVOLVE` link-related.ts → 8 hits ≥3.
