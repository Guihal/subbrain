...
### MEM-19 ✅ sleep-time focus block rewrite (закрыто M-11, 2026-04-26)
До M-11 `layer1_focus` (KV, инжектится в КАЖДЫЙ system prompt) обновлялся только вручную (UI / hippocampus `memory_write`). После M-06 reflect episodic→semantic поднимаются в shared, но focus сам не self-rewrites — Letta/MemGPT sleep-time pattern отсутствовал. Без него focus "затухает" относительно живой shared-памяти неделями.
**Fix:** M-11 — миграция 16 (`layer1_focus_shadow` KV mirror — без FTS / triggers). Новый night-cycle step `src/pipeline/night-cycle/steps/focus-rewrite.ts` (≤164 LOC, env-gated `NIGHT_CYCLE_FOCUS_REWRITE_ENABLED=true`, default off). Берёт editable focus keys (skip `PROTECTED_FOCUS_KEYS` — экспортирован из `prune/focus.ts`), top-K shared (`selectTopSharedForFocusRewrite`: persona×1.5 × (0.5+salience) × log(1+access_count), default K=30, status='active'), синтезирует новое value через `router.chat(NIGHT_CYCLE_MODEL,…,"low")` (default `memory` role). Skip semantics: identical-output / >MAX_FOCUS_LEN (default 500) / empty → `skipped++`; LLM throw → `errors++` (per-key, never aborts loop). Writes ТОЛЬКО в `layer1_focus_shadow` — real `layer1_focus` не trogano, `system-prompt.ts` reader неизменен. Wired в `post-steps.ts` ПОСЛЕ `pruneFocus` (rewrite не должен синтезировать удалённые ключи). `NightCycleResult` расширен `focusRewritten/focusSkipped/focusErrors`. `MemoryDB` facade + `MemoryRepository` + `MemoryTable` пробросы для `getShadowFocus / setShadowFocus / getAllShadowFocus / clearShadowFocus / selectTopSharedForFocusRewrite`.
...
### MEM-20 ✅ A-MEM evolution (закрыто M-05.1, 2026-04-27)
До M-05.1 `linkRelated` (M-05) рисовал `relates` edges на top-3 vec neighbours, но neighbours оставались immutable — их `tags` не обновлялись от нового контекста. A-MEM (Wang et al. 2024) описывает attribute evolution: при линковке neighbour absorbs novel attributes от inserter'а.
**Fix:** Pure string-merge в `src/pipeline/agent-pipeline/post/link-related.ts` (163 LOC, под self-cap 200). Module-private `evolveNeighbour(memory, neighbourId, neighbourLayer, insertedTags, cap)` после успешного `linkEdge` — читает current tags via `getContext/getShared`, merge unique (preserve order, новые в tail), tail-truncate до `LINK_EVOLVE_MAX_TAGS` (default 10), early-return если `sameSet(current, merged)` (no spurious updated_at bump). Helpers `parseTagsCsv` (exported для callers) / module-private `parseTags` / `mergeUnique` / `sameSet`. Wrap в try/catch — log.warn on throw, never abort linkRelated. Env knobs `LINK_EVOLVE_TAGS_ENABLED` (default true) / `LINK_EVOLVE_MAX_TAGS` (default 10), read at call-time. NO migration / NO schema change / NO LLM call / только tags evolve (content/confidence/kind/status untouched). `extractors.ts` writeShared/writeContext передают inserted tags (split CSV) в `linkRelated`.
Tests: `tests/post-link-related-evolution.test.ts` (8 cases: empty-tags, pure-add, dup-dedup, cap-eviction, no-op-already-covered, layer-routing-shared, env-disabled, neighbour-deleted-mid-flight). Post-merge на main: 787 pass / 0 fail × 2 runs (779 baseline + 8 new). tsc 0. grep `evolveNeighbour|LINK_EVOLVE` link-related.ts → 8 hits ≥3. (Worktree-only `usemarkdown.test.ts` 1 fail / 1 error был `isomorphic-dompurify` resolution FP — исчез после merge.)
**Out of scope:** tag-extraction LLM call, content evolution, edge weight from cosine (M-05.1 keeps weight=1.0 const), tag canonicalisation, A/B benchmark.
**Scope:** M-05.1.

### Memory-v2 M-05.1 review (2026-04-27, inline audit)
**Closed:** MEM-20 (M-05.1).
**§1 Debug findings:** 0 real regressions. `db.insertShared`: 1 hit `scripts/seed.ts:140` (pre-M-01 seed-script baseline, унаследовано от M-11 review). `'HIGH'/'LOW'` archive: 0 ✓. `console.*` в src/: 5 hits, все pre-existing baseline. M-05.1 diff: 0 `as any` / 0 `@ts-ignore` / 0 `Promise.all` non-Settled / 0 single-arg logger / 0 raw `fetch(` / 0 `TODO M-05.1`. ✓
**§2 File-cap:** `link-related.ts` 170 LOC (под self-cap 200, под repo cap 250 ✓; M-05 базис ~55, M-05.1 +~115). `extractors.ts` 274 LOC (over 250 — pre-existing; M-05.1 net +4 lines: import + 2 callsite). Без natural split.
**§3 Test stability:** на main `bun test` × 2 → 787 / 0 / 2381 expects identical (27.78s, 28.30s). Stable.
**§4 Schema:** unchanged — M-05.1 NO migration. `PRAGMA user_version = 16` (наследие M-11). ✓
**§5 Acceptance §3 worktree FP:** subagent видел 771/1/1 в worktree из-за `isomorphic-dompurify` module-resolution для `web/app/composables/useMarkdown.ts` (worktree node_modules subpath quirk). Post-merge на main: 787/0/2381 — FP исчез, acceptance "≥784 / 0 fail" satisfied.
**Open follow-ups:** M-05.2 (LLM contradiction detection — auto kind='contradicts' edges), M-09 (cross-layer dedup uses evolved tag-corpus). File-cap `extractors.ts` 274 LOC (M-FINAL3 candidate, pre-existing).
**Verdict:** M-05.1 закрыт чисто. 0 регрессов введено.
**Scope:** M-05.1 inline audit.

### MEM-21 ✅ contradiction detection (закрыто M-05.2, 2026-04-27)
M-05 положил schema (`memory_edges` CHECK включает `'contradicts'`), M-05.1 — `relates` + tag evolution. Авто-проставление `contradicts` оставалось дырой: `kind='contradicts'` edges были возможны только через ручную curation. M-05.2 закрывает: один LLM-call после relates-loop, JSON verdicts → `contradicts` edges (weight = LLM confidence).
**Fix:** В `src/pipeline/agent-pipeline/post/link-related.ts` (225 LOC, под self-cap 230) module-private `detectContradictions(router, log, content, candidates)` — single non-streaming `router.chat(<role>,messages,"low")` с system prompt "reply ONLY in JSON `{contradicts:[{id,confidence}]}`". Caps: insertedContent.slice(0,800) / candidate.content.slice(0,400). Bad JSON / throw → log.warn + return `[]`. Helpers `detectEnabled` / `minConf` / `contradictModel` (env at call-time). Сигнатура `linkRelated` гейтает `router: ModelRouter` (position 3, после rag). После relates-loop собирается `drawnNeighbours` (push только при успешном linkEdge), затем re-fetch content → LLM call → для verdicts ≥ threshold (default 0.7) `memory.linkEdge(insertedId, layer, dst.id, dst.layer, "contradicts", confidence)`. Best-effort outer try/catch — никогда не abort linkRelated. NO migration. NO content/status mutations. relates weight = 1.0 const, contradicts weight = LLM confidence.
**Caller cascade:** `extractors.ts` (writeShared / writeContext) added `router: ModelRouter` в обе сигнатуры. `hippocampus.ts` (uniq callsite) пробросил уже-доступный `router`. 7 test-файлов получили router stub в writeShared/writeContext callers (mechanical pass-through).
**Env knobs:** `LINK_CONTRADICT_ENABLED` (default `false` — opt-in! cost-control), `LINK_CONTRADICT_MIN_CONF` (default 0.7), `CONTRADICT_MODEL_ROLE` (default `memory`).
...
### MEM-22 ✅ MemoryService.linkRelated wiring (закрыто M-13, 2026-04-27)
M-05/M-05.1/M-05.2 (relates edges + tag evolution + contradiction detection) активировались только через `extractors.writeShared/writeContext` (hippocampus path). `MemoryService.insertShared/insertContext` — single-source-of-truth для embed-first transactional writes под MCP `memory_write`, night-cycle reflect/cross-layer-dedup промоушены, admin REST POST — НЕ вызывал `linkRelated`. Empirically подтверждено: после `memoryService.insertShared(...)` `getEdgesFromSrc(newId,"shared")` = 0.
**Fix:** `src/services/memory.service.ts` — расширен ctor двумя optional аргументами после `logRepo`: `memoryDb: MemoryDB | null = null` (нужен `linkRelated`'у для `linkEdge`/`getContext`/`getShared`/`update*`) и `linkDeps: MemoryServiceLinkDeps | null = null` (`{router, log}`). Приватный `runLinkRelated(id, layer, content, tagsCsv)` вызывается в обоих `insertShared` и `insertContext` сразу после `repo.transaction()`. Best-effort try/catch (warn-on-throw, никогда не abort write). `src/app/deps.ts` — production wiring `new MemoryService(memory.memoryRepo, rag, memory.logRepo, memory, { router, log: logger.forRequest("memory-svc","memory-svc") })`. Synthetic RequestLogger ("memory-svc"), т.к. сервис долгоживущий не request-scoped.
**Back-compat:** все existing 3-arg test callers (`tests/memory-kind.test.ts`, `tests/mcp-curation-tools.test.ts`, `tests/memory-routes-*.test.ts` etc.) работают без правок — defaults `null` дают skip-post-hook поведение.
Tests: `tests/memory-service-link-related.test.ts` (5 cases, ~210 LOC) — legacy 3-arg → 0 edges; full ctor → ≥1 relates edge к seed; M-05.1 tag evolution (a,b + c,d → a,b,c,d) propagates; throw в LLM router (LINK_CONTRADICT_ENABLED=true + mkThrowingRouter) не abort write; insertContext mirror.
**Acceptance:** `bunx tsc --noEmit` exit 0 ✓. `bun test tests/memory-service-link-related.test.ts` → 5/5 ✓. `bun test` → 800 pass / 0 fail (795 baseline + 5 new). `grep -nE "linkDeps|linkRelated" src/services/memory.service.ts` → 13 ≥4 ✓. `grep -n "linkDeps:" src/app/deps.ts` → 1 ≥1 ✓.
**File-cap:** `memory.service.ts` 362 LOC (был 305; net +57). Pre-existing >250 violation унаследован — задокументировано в плане §Файлы. Не split в M-13 (scope-lock).
**Out of scope:** refactor `linkRelated` под `MemoryRepository` (M-05.2 frozen artifact); admin REST `/v1/memory/edges` (отдельный M-14); UI surface для edges (M-14); `MemoryTools.writeSharedAtomic` legacy fallback fix; `context-compressor.ts:258` direct `memory.insertShared` call; backfill edges существующих rows.
**Scope:** M-13.

### MEM-23 ✅ edges admin surface (закрыто M-14, 2026-04-27)
До M-14 `memory_edges` (M-05/M-05.1/M-05.2) были невидимы для куратора: `src/routes/memory.ts` 0 endpoints, `web/app/` 0 references — relates-граф и contradicts-конфликты жили только в DB. M-14 закрывает read-only admin surface.
**Fix:** `src/routes/memory.ts` (+2 GET под `authMiddleware`, через `paginate(loader, query)`): `GET /v1/memory/edges?from=<id>&fromLayer=<context|shared|archive>[&kinds=csv]` → outbound `EdgeRow[]`; `GET /v1/memory/edges/related?id=<id>&layer=<...>[&kinds=csv]` → 1-hop neighbours `{id, layer, kind, weight}[]`. TypeBox: `EDGE_LAYER` Union of 3 Literals, `kinds` CSV → module-private `parseKindsCsv` whitelist (`relates|derives|supersedes|contradicts`); bogus layer / missing required → 422. `MemoryService` (+18 LOC) — pass-through `getEdgesFromSrc` / `getRelatedDetailed` на `memoryDb` facade (3-arg test ctor с `memoryDb=null` → `[]`, back-compat). `useMemory.ts` — `fetchEdges(id, layer)` arrow на `/edges/related?page_size=100`. `MemoryRow.vue` — collapsible "🔗 ▸/▾" toggle (rows c `__kind ∈ {context,shared,archive}` и валидным `id`); lazy-load on first expand, cached в local `ref`; `kind === 'contradicts'` → `text-red-400`.
Tests: `tests/routes-memory-edges.test.ts` (227 LOC, 10 cases) — 2× auth-401, 2× 422 validation (bogus layer + missing required), list outbound + filter, empty envelope, related 1-hop (out + in), kind filter on related. Per-test fresh `data/test-mem14-edges.db`.
**Acceptance:** `bunx tsc --noEmit` exit 0 ✓. `bun test tests/routes-memory-edges.test.ts` → 10/10 ✓. `bun test` → 810 pass / 0 fail (800 baseline + 10 new). `grep -n "/edges" src/routes/memory.ts` → 2 ✓. `grep -n "fetchEdges\|edges" web/app/components/MemoryRow.vue` → 21 ≥1 ✓.
